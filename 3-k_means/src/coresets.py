...
def get_coreset(data, num_clusters, coreset_size, delta=0.1):
    ...
    while data_remain.shape[0] > num_single_samples:
        ...
        print('Shape of the coreset samples so far ', samples.shape)
        print('Shape of the remaining samples ', data_remain.shape)
    ...
    print('Shape of the final remaining samples ', data_remain.shape)

    samples = np.vstack((samples, data_remain))

    print('Shape of the final coreset samples ', samples.shape)
    ...
    closest_sample_idx = np.zeros(num_frames, dtype=np.int32)
    for i in xrange(num_frames):
        print("Calculating dists for point ", i)
        dists = scipy.spatial.distance.cdist(data[i:i+1, :], samples)
        min_dist = np.min(dists)
        min_idx = np.argmin(dists)
        min_dists[i] = min_dist
        closest_sample_idx[i] = int(min_idx)

    for i in xrange(num_frames):
        # for each datapoint, Ix[i] is the index of the coreset point
        # it is assigned to.
        print(i, "closest_sample_idx[i]: ", closest_sample_idx[i])
